...
trigger ASI_TnE_ClaimDetail_AfterUpdate on ASI_TnE_ClaimDetail__c (after update) {
    ...
    //for KR
    if(Global_RecordTypeCache.getRt(trigger.new[0].recordTypeId).developerName.contains('ASI_TnE_KR_Claim_Detail')){
        ASI_TnE_KR_ClaimDetailTriggerClass.afterUpdateMethod(trigger.new);
    }
    //for KR
