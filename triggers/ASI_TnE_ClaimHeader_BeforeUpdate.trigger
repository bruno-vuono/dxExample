trigger ASI_TnE_ClaimHeader_BeforeUpdate on ASI_TnE_ClaimHeader__c (before update) {
  ...
  Map<Id, RecordType> recordTypeMap = new Map<Id, RecordType>([select id, name, developerName from recordType 
    ...
    if (Global_RecordTypeCache.getRt(trigger.new[0].recordTypeId).developerName.contains('ASI_TnE_KR')){
        ASI_TnE_KR_BudgetChecking.BudgetCheck(trigger.new);
        ASI_TnE_KR_ApprovalStep approvalStep =new ASI_TnE_KR_ApprovalStep();
        approvalStep.approvalStep(trigger.new); 
        ASI_TnE_KR_ClaimHeaderTriggerClass.beforeUpdateMethod(trigger.new,Trigger.oldMap, 'ASI_TnE_Status__c');
        ASI_TnE_KR_ClaimHeaderTriggerClass.sharingWithApprover(trigger.new, Trigger.oldMap);
        ASI_TnE_KR_ClaimHeaderTriggerClass.beforeSubmitChecking(trigger.new, Trigger.oldMap);
        ASI_TnE_KR_ClaimHeaderTriggerClass.wrongSpent(trigger.new, trigger.oldMap);
    }
    //for KR    
    //For TnE Sea
