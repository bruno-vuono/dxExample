...
String profileName=[Select Id,Name from Profile where Id=:profileId].Name;
    ...
    Map<Id, RecordType> KRrecordTypeMap = new Map<Id, RecordType>
        ([select id from recordType where sobjectType = 'ASI_TnE_ClaimHeader__c' and developerName like 'ASI_TnE_KR%']);
    List<ASI_TnE_ClaimHeader__c> KRvalidClaimHeaders = new List<ASI_TnE_ClaimHeader__c>();
    
    for (ASI_TnE_ClaimHeader__c claimHeader : Trigger.oldMap.values())
    {         
     if (KRrecordTypeMap.containsKey(claimHeader.recordTypeId))
     {
         KRvalidClaimHeaders.add(claimHeader);
     }       
    }  
    
    if(KRvalidClaimHeaders.size()>0)
    {
        ASI_TnE_KR_ClaimHeaderTriggerClass.beforeDeleteMethod(Trigger.old, 'ASI_TnE_Status__c');       
    }
    ...
    if(Global_RecordTypeCache.getRt(trigger.old[0].recordTypeid).developerName.contains('ASI_TnE_HK')) {
        ASI_TnE_triggerClasses.add(new ASI_TnE_HK_TnEHeaderCheckDel());
    }
