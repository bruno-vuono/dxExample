...
LAT_MX_ClientPaymentPeriod__c))</formula>
    ...
    <rules>
        <fullName>Altera tipo de registro - Pedido enviado JDE</fullName>
        <active>false</active>
    ...
    <rules>
        ...
        <active>false</active>
        <criteriaItems>
            <field>LAT_Opportunity__c.LAT_CDOrderStatusCountry__c</field>
            <operation>equals</operation>
            <value>5</value>
        </criteriaItems>
    ...
    <rules>
        <fullName>Pedido com data de entrega alterada</fullName>
        <actions>
            <name>Prazo_de_entrega_nao_sera_cumprido</name>
            <type>Task</type>
        </actions>
        <active>false</active>
        <description>Se a data de entrega do pedido for alterada para uma data posterior, gerar atividade para o executivo. Somente para pedidos com itens em BACKORDER.</description>
        <formula>(RecordType.DeveloperName=&apos;Nova_oportunidade&apos; ||RecordType.DeveloperName=&apos;Bloqueia_alteracao_do_cabecalho&apos; ||RecordType.DeveloperName=&apos;Bloqueia_alteracao&apos;)  &amp;&amp;  ( AND(       LAT_BackorderItemExists__c = 1,       PRIORVALUE(LAT_DTDelivery__c) &lt; LAT_DTDelivery__c,       NOT(ISNULL(LAT_DTDelivery__c))          ) )</formula>
        <triggerType>onAllChanges</triggerType>
    </rules>
    <rules>
        <fullName>Pedido com data de entrega recalculada</fullName>
        <actions>
            <name>Itens_do_pedido_fora_da_data_de_entrega</name>
            <type>Task</type>
        </actions>
        <active>false</active>
        <description>Se o pedido não for atendido e a data recalculada de entrega (baseada no calendário PERNOD) for superior à data de entrega informada o executivo é avisado da nova data.</description>
        <formula>(RecordType.DeveloperName=&apos;Nova_oportunidade&apos; ||RecordType.DeveloperName=&apos;Bloqueia_alteracao_do_cabecalho&apos; ||RecordType.DeveloperName=&apos;Bloqueia_alteracao&apos;)  &amp;&amp;  ( AND (        NOT(ISNULL(LAT_ReEstimatedDeliveryDate__c)),        NOT(ISNULL(LAT_DTDelivery__c)),        LAT_DTDelivery__c  &lt; LAT_ReEstimatedDeliveryDate__c ) )</formula>
        <triggerType>onAllChanges</triggerType>
    </rules>
    <rules>
        <fullName>Pedido com erro no JDE</fullName>
        <actions>
            <name>Pedido_com_erro_de_envio_ao_JDE</name>
            <type>Alert</type>
        </actions>
        <active>false</active>
        <description>Envia email para executivo no caso de retorno de ERRO do pedido do JDE.</description>
        <formula>(RecordType.DeveloperName=&apos;Nova_oportunidade&apos; ||RecordType.DeveloperName=&apos;Bloqueia_alteracao_do_cabecalho&apos; ||RecordType.DeveloperName=&apos;Bloqueia_alteracao&apos;)  &amp;&amp;  ( AND(        NOT(               ISPICKVAL(PRIORVALUE(LAT_StageName__c),&apos;Pedido com erro no JDE&apos;)                 ),        ISPICKVAL(LAT_StageName__c,&apos;Pedido com erro no JDE&apos;)          ) )</formula>
        <triggerType>onAllChanges</triggerType>
    </rules>
    <rules>
